...
%     for lix = 1:numel(ls) % lick index for current trial
        p_cur = pfit{trix}; % current fit parameters for current trial and lick number
%         
...
%     end
    newlickR = interp1(pfit{trix, 1}, pfit{trix, 2}, lickR, 'linear');
    newlickL = interp1(pfit{trix, 1}, pfit{trix, 2}, lickL, 'linear');
    
    obj.bp.ev.lickR_warped{trix} = newlickR;
    obj.bp.ev.lickL_warped{trix} = newlickL;
